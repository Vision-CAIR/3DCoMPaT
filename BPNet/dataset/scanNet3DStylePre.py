...
class ScanNet3D(data.Dataset):
    ...
    def __init__(self, dataPathPrefix='Data', dataPathPrefix2D='/data/compat/2d_image', voxelSize=0.05,
        ...
        # print("")
        if memCacheInit and (not exists("/dev/shm/wbhu_scannet_3d_%s_%06d_locs_%08d" % (split, identifier, 1))):
            ...
            for i, model_id in enumerate(model_ids):
                ...
                locs, feats, labels, inds_reconstruct, index = self.voxelizer.prevoxelize(b[:, :3], b[:, 3:6], b[:, 6])
                #     SA.delete("shm://wbhu_scannet_3d_%s_%06d_locs_%08d" % (split, identifier, i))
                #     SA.delete("shm://wbhu_scannet_3d_%s_%06d_feats_%08d" % (split, identifier, i))
                #     SA.delete("shm://wbhu_scannet_3d_%s_%06d_labels_%08d" % (split, identifier, i))
                #     print(model_ids)

                sa_create("shm://wbhu_scannet_3d_%s_%06d_vlocs_%08d" % (split, identifier, j), locs)
                sa_create("shm://wbhu_scannet_3d_%s_%06d_vfeats_%08d" % (split, identifier, j), feats)
                sa_create("shm://wbhu_scannet_3d_%s_%06d_vlabels_%08d" % (split, identifier, j), labels)
                sa_create("shm://wbhu_scannet_3d_%s_%06d_inds_%08d" % (split, identifier, j), inds_reconstruct)
                sa_create("shm://wbhu_scannet_3d_%s_%06d_index_%08d" % (split, identifier, j), index)
