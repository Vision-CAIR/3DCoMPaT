...
class ScanNetCross(ScanNet3D):
    ...
    def __init__(self, dataPathPrefix='/data/dataset/glbfile',
                 dataPathPrefix2D='/data/compat/2d_image',
                 voxelSize=0.05,
                 split='train', aug=True, memCacheInit=True,
                 identifier=7806, loop=1,
    ...
    def __getitem__(self, index_long):
        ...
        print("before voxel shapes size: {}".format(locs.shape))
        # print("")
        np.save( '/home/liy0r/3d-few-shot/download/BPNet/dataset/outcls.npy', locs
                )
        locs, feats, labels, inds_reconstruct, links = self.voxelizer.voxelize(locs, feats_in, labels_in, link=links)
        np.save('/home/liy0r/3d-few-shot/download/BPNet/dataset/outcls1.npy', locs
                )
        ...
        print("after voxel shapes size: {}".format(locs.shape))
        ...
        if self.eval_all:
            return coords, feats, labels, colors, labels_2d, links, torch.from_numpy(inds_reconstruct).long(), category
        return coords, feats, labels, colors, labels_2d, links, category
...
def collation_fn(batch):
    ...
    """
    coords, feats, labels, colors, labels_2d, links, cls = list(zip(*batch))
    # pdb.set_trace()
    ...
    return torch.cat(coords), torch.cat(feats), torch.cat(labels), \
           torch.stack(colors), torch.stack(labels_2d), torch.cat(links), torch.stack(cls)
...
def collation_fn_eval_all(batch):
    ...
    """
    try:
        coords, feats, labels, colors, labels_2d, links, inds_recons, cls = list(zip(*batch))
    except:
    ...
    return torch.cat(coords), torch.cat(feats), torch.cat(labels), \
           torch.stack(colors), torch.stack(labels_2d), torch.cat(links), torch.cat(inds_recons), torch.stack(cls)
...
if __name__ == '__main__':
    ...
    coords, feats, labels, colors, labels_2d, links, inds_recons, cls = val_data.__getitem__(1)
    ...
    # import mayavi.mlab
    # nodes = mayavi.mlab.points3d(coords[1],coords[2],coords[3], mode="cube", scale_factor=0.8)
    np.save("/home/liy0r/3d-few-shot/download/BPNet/dataset/arr.npy", coords)
    # nodes.glyph.scale_mode = 'scale_by_vector'
    # nodes.mlab_source.dataset.point_data.scalars = labels
    #
    # mayavi.mlab.savefig('mlab_contour3d.png')
    # # def showVoxel(voxel):
    #     fig = plt.figure()
    ...
    #     #     end = time.time()
    #
    #     for step, (coords, feats, labels, colors, labels_2d, links, inds_reverse, cls) in enumerate(val_loader):
    #         print(
