...
class ScanNetCross(ScanNet3D):
    ...
    def __init__(self, dataPathPrefix='/ibex/scratch/liy0r/cvpr/glbfile',
        ...
        lines = []
        stylev0 = (np.genfromtxt('data/style1v0.txt', dtype='str'))
        stylev1 = (np.genfromtxt('data/style1v1.txt', dtype='str'))

        # # the training val test split
        # model_ids = defaultdict(list)
        # with open(test_dir + "split.txt", "r") as f:
        #     for line in f:
        #         ids, label = line.rstrip().split(',')
        #         model_ids[label].append(ids)

        df = pd.read_csv("/ibex/scratch/liy0r/3dcompat/3dcompat/data/meta-data/materials_df.csv")
        df = df.loc[:, ["name", "type"]]
        ...
        # root_mat = "/ibex/scratch/projects/c2090/tezuesh/mat_part"
        self.test_dir = test_dir
        # jj = 0
        # df = pd.read_csv('data/part_mat.csv')
        #
        # part_mat = dict(zip(df['model_id'], df['tuples']))
        # for i in part_mat:
        #     part_mat[i] = json.loads(part_mat[i])
    ...
    def get_model_ids(self):
        test_dir = '/ibex/scratch/liy0r/cvpr/BPNet/'
        df = pd.read_csv(test_dir + "data/model.csv")
