...
class ScanNet3D(data.Dataset):
    ...
    def __init__(self, dataPathPrefix='Data', voxelSize=0.05,
        ...
        if memCacheInit and (not exists("/dev/shm/wbhu_scannet_3d_%s_%06d_locs_%08d" % (split, identifier, 0))):
            ...
            indexs = []
            j = 0
            for i, model_id in enumerate(self.data_paths):
                ...
                sa_create("shm://wbhu_scannet_3d_%s_%06d_locs_%08d" % (split, identifier, j), sample_xyz)
                sa_create("shm://wbhu_scannet_3d_%s_%06d_feats_%08d" % (split, identifier, j), sample_colors)
                sa_create("shm://wbhu_scannet_3d_%s_%06d_labels_%08d" % (split, identifier, j), sample_segment)
                indexs.append(model_id)
                self.part_materials[model_id] = j
                j += 1
            self.length = j
            print(self.length)
            print(j)
            ...
            with open("indexs_{}.txt".format(split), "r") as f:
                ...
                for line in f:
                    ...
                    self.part_materials[model_id] = j
                    j += 1
                    self.data_paths.append(model_id)
                    # model_ids[label].append(ids)
                    # print(len(self.part_materials))
