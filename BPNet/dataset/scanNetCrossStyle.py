...
class ScanNetCross(ScanNet3D):
    ...
    def __init__(self, dataPathPrefix='/ibex/scratch/liy0r/cvpr/glbfile',
        ...
        lines = []
        #50 compositions for each models
        filename = '50_styles.txt'

        # material part info from each compostion
    ...
    def get_2d(self, room_id, coords: np.ndarray):
        ...
        for v in range(self.VIEW_NUM):
            ...
            mat_cat = [0, 6, 38, 39, 55, 58, 63, 73, 87, 97, 110, 145, 153]
            for i, x in enumerate(mat_cat):
                for j in range(mat_cat[i], mat_cat[i + 1]):
                    self.remapper[x] = i
                if i == 11:
                    break
...
if __name__ == '__main__':
    ...
    for idx in range(1):
        ...
        for step, (coords, feats, labels, colors, labels_2d, links, inds_reverse, categories, mat) in enumerate(
                val_loader):
            print(
