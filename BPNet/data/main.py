...
class PartDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        part_boxes = []
        for bb in partname_bb.values():
            part_boxes.append(bb)
        
        part_boxes = torch.tensor(part_boxes)
        label_part =  np.zeros(len(part_boxes), dtype=np.int32)
        ...
        label_obj =  np.zeros(len(obj_bb), dtype=np.int32)
        ...
        # Material Param -------------------------------------------------------
        boxes = []
        for bb in material_bb.values():
            boxes.append(bb)
        
        boxes = torch.tensor(boxes)        
        label_mat =  np.zeros(len(boxes), dtype=np.int32)
...
def run(fold):
    ...
    train_data_loader = DataLoader(valid_dataset,batch_size=BATCH_SIZE,shuffle=False,num_workers=4,collate_fn=collate_fn)
