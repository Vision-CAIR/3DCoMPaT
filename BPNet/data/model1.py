...
class PartDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        for key, value in partname_bb.items():
            part_boxes.append(box_xywh_to_cxcywh(value))
            # print(value)
            if key not in parts:
        ...
        obj_bb = torch.tensor([box_xywh_to_cxcywh([part_boxes[:,0].min(), part_boxes[:1].min(), w, h])])
        ...
        for key, value in material_bb.items():
            mat_boxes.append(box_xywh_to_cxcywh(value))
            if key not in parts:
...
class DETRModel(nn.Module):
    ...
    def forward(self,images):
        ...
        return {'pred_logits_part': self.model_part(features)[-1], 
                'pred_logits_mat': self.model_ma(features)[-1],
                'pred_logits_obj':self.model_obj(features)[-1],
                'pred_boxes_pa_ma': self.bbox_part_ma(features).sigmoid()[-1],
                'pred_boxes_obj': self.bbox_obj(features).sigmoid()[-1]}
...
def train_fn(data_loader,model,criterion,optimizer,device,scheduler,epoch):
    ...
    for step, (images, target_part, target_obj, target_mat, image_ids, obj_class) in enumerate(tk0):
        ...
        mat_log = output['pred_logits_mat'].reshape(-1, 100, num_cl_mat)
        obj_log = output['pred_logits_obj'].reshape(-1, 100, num_cl_obj)
        bb_obj = output['pred_boxes_obj'].reshape(-1, 100, 4)
...
def run(fold):
    ...
    print("time taken for dataloader ", end-start)
    ...
    for epoch in range(EPOCHS):
        ...
        if valid_loss.avg < best_loss:
            ...
            torch.save(model.state_dict(), f'model1_{fold}.pth')
