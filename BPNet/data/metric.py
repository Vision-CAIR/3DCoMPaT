...
class BboxEval:
    def __init__(self):
        ...
        self.per_obj_parts_correct = defaultdict(float) # Number of (part, mat) pair predicted correctly for a given verb
    ...
    def update(self, pred_obj, pred_mat, pred_part, pred_bboxes, gt_obj, gt_mat, gt_part, gt_bboxes):
        print(gt_obj.shape, pred_obj.shape)
        order = gt_part
        ...
        for i, pred in enumerate(zip(pred_part, pred_mat)):
            if pred in zip(gt_part, gt_mat):
            ...
            if pred in zip(gt_part, gt_mat) :
                j = get_corresponding_box(pred, zip(gt_part, gt_mat))
