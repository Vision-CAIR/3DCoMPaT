...
def train_cross(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, batch_data in enumerate(train_loader):
        ...
        if args.data_name == 'scannet_cross':
            ...
            mat = mat.cuda(non_blocking=True)
            mat_3d = mat_3d.cuda(non_blocking=True)
            output_3d, output_2d = model(sinput, color, link, to_categorical(cls, 43))
    ...
    with torch.no_grad():
        ...
        for i, batch_data in enumerate(val_loader):
            if args.data_name == 'scannet_cross':
                ...
                output_3dmat, output_mat = model(sinput, color, link, cls)
                # output_3d = output_3d[inds_reverse, :]
            else:
                raise NotImplemented

            loss_mat = criterion(output_mat, label_2d)
            loss_3dmat = criterion(output_3dmat, label_3d)
            ...
            intersection, union, target = intersectionAndUnionGPU(output_3dmat, label_3d.detach(), args.mat,
                                                                  args.ignore_label)
            intersection, union, target = intersection.cpu().numpy(), union.cpu().numpy(), target.cpu().numpy()
            ...
            # ############ mat ############ #
            output_mat = output_mat.detach().max(1)[1]
            intersection, union, target = intersectionAndUnionGPU(output_mat, label_2d.detach(), args.mat,
                                                                  args.ignore_label)
            if args.multiprocessing_distributed:
                dist.all_reduce(intersection), dist.all_reduce(union), dist.all_reduce(target)
            intersection, union, target = intersection.cpu().numpy(), union.cpu().numpy(), target.cpu().numpy()
