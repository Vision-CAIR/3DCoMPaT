...
def main():
    ...
    if args.data_name == 'unet_2d':
        from dataset.scanNet2DClsPre import ScanNet3D, collation_fn
        ...
        from dataset.scanNetCrossCls import ScanNetCross, collation_fn, collation_fn_eval_all
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.sync_bn_2d:
        ...
        model.layer0 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model.layer4)
        model.up4 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model.up4)
        model.delayer4 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model.delayer4)
        model.up3 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model.up3)
        model.delayer3 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model.delayer3)
        model.up2 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model.up2)
        model.delayer2 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model.delayer2)
        model.cls = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model.cls)
    ...
    # ####################### Data Loader ####################### #
    if args.data_name == 'unet_2d':
        from dataset.scanNet2DClsPre import ScanNet3D, collation_fn, collation_fn_eval_all
        ...
        from dataset.scanNet2DCls import ScanNetCross, collation_fn, collation_fn_eval_all
        ...
        if args.evaluate:
            val_data = ScanNetCross(dataPathPrefix=args.data_root, dataPathPrefix2D=args.data_root2d,
                                    voxelSize=args.voxelSize, split='val', aug=False,
                                    memCacheInit=True, eval_all=True)
            val_sampler = torch.utils.data.distributed.DistributedSampler(val_data) if args.distributed else None
            val_loader = torch.utils.data.DataLoader(val_data, batch_size=args.batch_size_val,
                                                     shuffle=False, num_workers=args.workers, pin_memory=True,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.data_name == 'scannet_cross':
            ...
            loss_train, mIoU_train, mAcc_train, allAcc_train = train_cross(train_loader, model, criterion, optimizer,
                                                                           epoch)
        epoch_log = epoch + 1
        if main_process():
        ...
        if args.evaluate and (epoch_log % args.eval_freq == 0):
            if args.data_name == 'scannet_cross':
                ...
                loss_val, mIoU_val, mAcc_val, allAcc_val = validate_cross(val_loader, model, criterion)
...
def get_model(cfg):
    if cfg.arch == 'unet_2d':
        from models.unet_2d import ResUnet as Model
        model = Model(layers=34, classes=90, cfg=cfg)
    elif cfg.arch == 'mink_34C':
        from models.unet_3d import MinkUNet34C as Model
        model = Model(in_channels=3, out_channels=20, D=3)
    elif cfg.arch == 'bpnet':
        from models.bpnetCls import BPNet as Model
        model = Model(cfg=cfg)
    else:
...
def train_cross(train_loader, model, criterion, optimizer, epoch):
    ...
    loss_meter, loss_meter_2d, loss_meter_cls = AverageMeter(), AverageMeter(), AverageMeter()
    ...
    for i, batch_data in enumerate(train_loader):
        ...
        loss_2d = criterion(output_2d, seg)
        loss = loss_2d + loss_cls + loss_mat
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info('Epoch: [{}/{}][{}/{}] '
                        ...
                        'Batch {batch_time.val:.3f} ({batch_time.avg:.3f}) '
                        'Remain {remain_time} '
                        'MATLoss {loss_meter.val:.4f} '
                        'MATAccuracy {accuracy:.4f}'
                        'Loss {loss_meter_2d.val:.4f}'
                        'Accuracy2D {accuracy_2d:.4f}'
                        'Acc Cls {acc:.4f}.'.format(epoch + 1, args.epochs, i + 1, len(train_loader),
                                                    batch_time=batch_time, data_time=data_time,
                                                    remain_time=remain_time,
                                                    loss_meter=loss_meter_mat,
        ...
        if main_process():
            ...
            writer.add_scalar('mAcc2d_train_batch', np.mean(intersection_meter_2d.val / (target_meter_2d.val + 1e-10)),
                              current_iter)
            writer.add_scalar('mIoUmat_train_batch',
                              np.mean(intersection_meter_mat.val / (union_meter_mat.val + 1e-10)),
                              current_iter)
            writer.add_scalar('mAccmat_train_batch',
                              np.mean(intersection_meter_mat.val / (target_meter_mat.val + 1e-10)),
                              current_iter)
            writer.add_scalar('allAcc2d_train_batch', accuracy_2d, current_iter)

            writer.add_scalar('allAccmat_train_batch', accuracy_mat, current_iter)
    ...
    if main_process():
        logger.info(
            'Train result 2d at epoch [{}/{}]: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.'.format(epoch + 1, args.epochs,
                                                                                              ...
                                                                                              allAcc_2d))
        logger.info(
            'Train result mat at epoch [{}/{}]: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.'.format(epoch + 1, args.epochs,
                                                                                               mIoU_mat, mAcc_mat,
                                                                                               allAcc_mat))
    ...
    return loss_meter_2d.avg, mIoU_2d, mAcc_2d, allAcc_2d


def validate_cross(val_loader, model, criterion):
    torch.backends.cudnn.enabled = False  # for cudnn bug at https://github.com/pytorch/pytorch/issues/4107
    loss_meter, loss_meter_2d, loss_meter_cls, loss_meter_mat = AverageMeter(), AverageMeter(), AverageMeter(), AverageMeter()
    intersection_meter_2d = AverageMeter()
    union_meter_2d = AverageMeter()
    target_meter_2d = AverageMeter()
    target_meter_mat, union_meter_mat, intersection_meter_mat = AverageMeter(), AverageMeter(), AverageMeter()
    ...
    if main_process():
        ...
        logger.info(
            'Val mat result: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.'.format(mIoU_mat, mAcc_mat, allAcc_mat))
