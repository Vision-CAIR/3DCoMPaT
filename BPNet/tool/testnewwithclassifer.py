...
def validate_cross(model, val_loader):
    ...
    with torch.no_grad():
        ...
        for i, batch_data in enumerate(tqdm(val_loader)):
            ...
            # for a, b, c, d, e, f in zip(output_2d.cpu().numpy(), output_mat.cpu().numpy(), ocls.cpu().numpy(), label_2d.cpu().numpy(),  mat.cpu().numpy(), cls.cpu().numpy()):
            #     pred_part = np.unique(a)
            #     pred_mat = np.unique(b)
            #     gt_mat = np.unique(e)
            #     gt_part = np.unique(d)
