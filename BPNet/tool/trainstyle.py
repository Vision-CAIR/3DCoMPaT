...
def train_cross(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, batch_data in enumerate(train_loader):
        data_time.update(time.time() - end)
        if args.data_name == 'scannet_cross':
            (coords, feat, label_3d, color, label_2d, link, cls, mat) = batch_data
            # For some networks, making the network invariant to even, odd coords is important
        ...
        # Adjust lr
        current_iter = epoch * len(train_loader) + i + 1
        print("current iter : {}".format(current_iter))

        print("base lr: {}".format(args.base_lr))
        print("power{}".format(args.power))
...
def validate_cross(val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for i, batch_data in enumerate(val_loader):
            ...
            # outseg3d.append(output_3d.detach_().cpu())
