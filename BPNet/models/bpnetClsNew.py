...
class BPNet(nn.Module):
    ...
    def forward(self, sparse_3d, images, links):
        ...
        x6 = torch.max(x5.view(x_size[0], x_size[4], x5.shape[-3], x5.shape[-2], x5.shape[-1]), 1)[0].view(x_size[0],
                                                                                                           -1)
        # x6 = torch.flatten(F.max_pool2d(x5, (1, 1)), 1)
        ...
        # out_b5 = torch.flatten(F.max_pool3d(out_b4p16, (1, 1), 1))
        # categories_3d = F.linear(out_b5.shape[1], self.categories)
        # Linking @ p5
        V_B, C, H, W = x5.shape
        ...
        # res_mat_3d = self.fc_mat3d(res_3d)
        ...
        if self.style:
            ...
            res_mat = res_mat.view(self.viewNum, -1, C, H, W).permute(1, 2, 3, 4, 0)

            return res_3d.F, res_2d, categories_2d, res_mat, res_3dmat.F
        return res_3d.F, res_2d, categories_2d
